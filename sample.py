...
def main():
    ...
    color = ['black', 'red', 'blue', 'green', 'orange', 'cyan', 'tomato', 'magenta', 'purple', 'brown', 'aqua', 'azure',
             'beige', 'brown', 'coral', 'deeppink', 'fuchsia', 'gold', 'ivory', 'lavender', 'lime', 'olive']
    ...
    for category in range(len(model_params.categories)):
        ...
        _, pngs, labels, _ = test_set._get_batch_from_indices(index[0:samples_per_category])
        ...
        i_label, j_label = sess.run([model.i_label, model.j_label], feed)
        ...
        for cnt in range(samples_per_category):
            ...
            filepath3 = os.path.join(SVG_DIR, 'i_label_%d_%d.npy' % (category, cnt))
            np.save(filepath3, i_label[cnt])
            filepath4 = os.path.join(SVG_DIR, 'j_label_%d_%d.npy' % (category, cnt))
            np.save(filepath4, j_label[cnt])
